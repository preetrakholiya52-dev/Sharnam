...
const LanguageForm = () => {
      ...
      const fetchLanguage = async () => {
        try {
          ...
          if (response.data.status) {
            setFormData({
              lang: response.data.result.lang || '',
  ...
  const handleChange = (e) => {
    ...
    if (name === 'isStatus') {
      // Handle select element for status
      setFormData(prev => ({
        ...prev,
        [name]: parseInt(value)
      }));
  ...
  return (
    <div className="w-full max-w-4xl mx-auto pb-xl">
      <div className="flex items-center gap-md mb-lg">
        <Link to="/admin/languages" className="p-xs bg-surface-container rounded-full hover:bg-[#e0bfbc] transition-colors text-on-surface">
          <ArrowLeft size={20} />
        </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div className="relative w-full">
              ...
              />
              <label htmlFor="lang" className="absolute text-on-surface-variant left-md top-1/2 -translate-y-1/2 transition-all duration-200 pointer-events-none origin-left text-body-md font-body-md">
                Language Name *
              </label>
            </div>
