...
const LanguageList = () => {
  ...
  const handleDelete = (id) => {
    toast((t) => (
      <div className="flex flex-col gap-xs">
        <p className="font-body-md m-0">Are you sure you want to delete this language?</p>
      ...
      render: (row) => (
        <div className="flex gap-sm">
          {permissions.isEdit === 1 && (
            <Link to={`/admin/languages/edit/${row.id}`} className="p-xs text-primary hover:bg-surface-container rounded transition-colors">
              <Edit size={18} />
            </Link>
          )}
  ...
  if (loading) {
    return <div className="p-xl text-center font-body-md">Loading languages...</div>;
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex justify-between items-center mb-lg">
        <div>
          <h2 className="font-headline-md text-headline-md text-on-surface">Manage Languages</h2>
          <p className="font-body-md text-on-surface-variant mt-xs">View, search, and manage languages.</p>
        </div>
      ...
      <div className="flex-1 overflow-hidden">
        <DataTable columns={columns} data={languages} exportFileName="LuxCare_Languages" />
      </div>
    </div>
  );
};
