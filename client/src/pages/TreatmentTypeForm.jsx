...
const TreatmentTypeForm = () => {
      ...
      const fetchTreatmentType = async () => {
        try {
          ...
          if (response.data.status) {
            setFormData({
              treatType: response.data.result.treatType || '',
              isStatus: response.data.result.isStatus ?? 1
            });
          }
        } catch (err) {
          setError('Failed to load treatment type details');
          console.error(err);
        } finally {
          setLoading(false);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await api.put(`/treatment-types/${id}`, formData);
      } else {
        await api.post('/treatment-types', formData);
      }
      navigate('/admin/treatment-types');
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto pb-xl">
      <div className="flex items-center gap-md mb-lg">
        <Link to="/admin/treatment-types" className="p-xs bg-surface-container rounded-full hover:bg-[#e0bfbc] transition-colors text-on-surface">
          <ArrowLeft size={20} />
        </Link>
        <h2 className="font-headline-md text-headline-md text-on-surface">
          {isEditMode ? 'Edit Treatment Type' : 'Add New Treatment Type'}
        </h2>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
            ...
            <div className="relative w-full">
              ...
              />
              <label htmlFor="treatType" className="absolute text-on-surface-variant left-md top-1/2 -translate-y-1/2 transition-all duration-200 pointer-events-none origin-left text-body-md font-body-md">
                Treatment Type Name *
              </label>
            </div>
          ...
          <div className="flex justify-end border-t border-[#E7E7E7] pt-lg mt-xl">
            ...
            >
              <Save size={18} />
              {saving ? 'Saving...' : 'Save Treatment Type'}
            </button>
          </div>
        </form>
      </div>
